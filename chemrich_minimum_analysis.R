...
run_chemrich_basic <- function(inputfile, 
                               object = FALSE, 
                               save_output = FALSE) {
  # ndf <- readxl::read_xlsx(path = inputfile, sheet = "Sheet1")
  if(object){
    ndf <- inputfile
  }else{
    ndf <- read.delim(inputfile)
  }
  ...
  names(clustdf.e) <- c("cluster_name","cluster_size","pvalues","FDR","key_compound","altered_metabolites","increased","decreased","increased_ratio","altered_ratio")
